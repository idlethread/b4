...
SYNOPSIS
--------
b4 {mbox,am,shazam,pr,diff,ty,kr,dig,integrate,review,prep,send,trailers} [options]
...
* *dig*: Find the mailing list submission for a given commit
* *integrate*: Batch-build integration branches from tracked series
...
``b4 mbox --rethread [msgid1] [msgid2] [...]``
  Stitch multiple unthreaded messages into a single series and save as
  an mbox file. Useful when a submitter sends patches without proper
  threading.
...
``b4 am --rethread [msgid1] [msgid2] [...]``
  Stitch multiple unthreaded messages into a single series and prepare
  them for ``git am``.
...
When using ``-H``, you can pass ``--resolve`` to interactively resolve
any merge conflicts using a three-way merge.
...
b4 dig
~~~~~~
Given a commit in a local git repository, this command tries to find
the original mailing list submission on a public-inbox instance. It
uses several strategies in order of reliability:

* match by stable patch-id (tries multiple diff algorithms)
* match by author email and subject
* match by in-body From: line (for patches sent from a different
  address than the commit author)

If ``--all`` is specified, b4 will also try to locate all other
revisions of the same series using change-id or subject matching.

For options and their descriptions, see ``b4 dig --help``.

*Examples*

``b4 dig [commit]``
  Find the mailing list submission for the given commit and print a
  link to it.

``b4 dig -a [commit]``
  Find all revisions of the series containing the given commit.

``b4 dig -w [commit]``
  Find the submission and show who was originally included in To/Cc.

``b4 dig -m /tmp/thread.mbx [commit]``
  Find the submission and save the full thread as an mbox file.

b4 integrate
~~~~~~~~~~~~
This command rebuilds a set of git branches from lists of in-flight patch
series tracked by message-id, applying each series with ``b4 shazam``. It
is aimed at maintainers who assemble an integration tree out of several
series that are still being revised on the mailing list.

The configuration is a YAML file mapping each branch name to an ordered
list of message-ids. For each branch, b4 checks out the base ref (``HEAD``
by default, or ``--base``), creates the branch with ``git checkout -B``,
and applies each message-id in order. Branches are built independently, so
a failure in one does not affect the others.

The working tree is treated as scratch space: b4 records the current branch
and commit before starting and always restores that snapshot after each
branch (aborting any in-progress ``git am``, checking out the original ref,
``git reset --hard``, and ``git clean -fd``). Only run it in a repository
whose working tree you are willing to have reset.

On an apply failure, b4 offers an interactive menu to continue (after a
manual fix), retry, skip the message-id, or abort the branch. In a
non-interactive run the branch is aborted instead of blocking on input.

With ``--update-config``, b4 looks for a newer revision of each series on
the public-inbox server after it applies and rewrites the YAML file in
place with any newer message-ids.

For options and their descriptions, see ``b4 integrate --help``.

*Examples*

``b4 integrate series.yaml``
  Rebuild every branch described in ``series.yaml`` on top of the current
  ``HEAD``.

``b4 integrate series.yaml --base v6.14-rc1``
  Same, but base every branch on the ``v6.14-rc1`` tag instead of ``HEAD``.

``b4 integrate series.yaml --update-config``
  Rebuild the branches and rewrite ``series.yaml`` to point at the latest
...
b4 review (alpha)
~~~~~~~~~~~~~~~~~
**Note:** this subcommand is a technology preview. Commands, flags, and
on-disk formats may change in incompatible ways between releases.

This command provides a TUI-based review workflow for maintainers
receiving patches on mailing lists. It lets you track incoming series,
review diffs with inline commenting, add code-review trailers, send
review emails, and take (apply) patches.

The workflow consists of three steps: enrol a repository, track one or
more series, and launch the TUI to review and act on them.

For options and their descriptions, see ``b4 review --help`` and
``b4 review <subcommand> --help``.

*Examples*

``b4 review enroll``
  Enrol the current repository for review tracking.

``b4 review track <msgid>``
  Track a series by its message-id, URL, or change-id.

``b4 review track --rethread <msgid1> <msgid2> [...]``
  Track a series that was sent without proper threading by stitching
  the given messages into a single series.

``b4 review forget <msgid>``
  Completely erase all records of a tracked series, identified by any
  of its message-ids, a URL, or its change-id. Prints a summary of the
  matched series and asks for confirmation before erasing. Works on
  any tracked series regardless of status, including archived ones the
  TUI no longer shows.
